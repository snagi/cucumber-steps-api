...
defineStep(
  ...
  function (method, url, type, kind, content) {
    ...
    return this.client
      ...
      .method(method)
      .type(type)
      .body(Template.fromTemplateContent(content, kind || 'js', null, this.generator))
      .send();
  }
);
defineStep(
  ...
  function (method, url, type, filename) {
    ...
    return this.client
      ...
      .method(method)
      .type(type)
      .body(Template.fromTemplateFile(this.resourceResolver(filename), null, this.generator))
      .send();
  }
);
defineStep(
...
defineStep(/I expect last response to have status as "([^"]+)"/, function (status) {
...
defineStep(/I log the response/, function () {
  ...
  this.attach(JSON.stringify(this.store.get(LAST_RESPONSE).body, null, 2), 'application/json');
});
...
defineStep(/I log the request and response/, function () {
  ...
  this.attach('Request -');
  this.attach(JSON.stringify(this.store.get(LAST_REQUEST).body, null, 2), 'application/json');
  debug(JSON.stringify(this.store.get(LAST_RESPONSE).body));
  this.attach('Response -');
  this.attach(JSON.stringify(this.store.get(LAST_RESPONSE).body, null, 2), 'application/json');
});
